...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/backend/system/user/add'
    });
...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/backend/system/user/edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.roleAssign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/backend/system/user/roleAssign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        var operation = function(){
            ...
            $.ajax({
                type: "post",
                url:  Feng.ctxPath + "/backend/system/user/delete",
                dataType: "json",
                data:{ids:userId},
                async: true,
...
$(function () {
    ...
    var table = new BSTable("managerTable", "/backend/system/user/list", defaultColunms);
