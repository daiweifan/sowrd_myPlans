...
Role.initColumn = function () {
    var columns = [
        ...
        {title: '名称名称', field: 'name', align: 'center', valign: 'middle', sortable: true},
        {title: '角色编码', field: 'code', align: 'center', valign: 'middle', sortable: true},
...
Role.openAddRole = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/backend/system/role/add'
    });
...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/backend/system/role/edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/backend/system/role/delete", function () {
                Feng.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/backend/system/role/assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
$(function () {
    ...
    var table = new BSTable(Role.id, "/backend/system/role/list", defaultColunms);
